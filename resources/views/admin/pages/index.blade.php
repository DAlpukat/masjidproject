        ...
        <div class="bg-white p-4 rounded shadow mb-6">
            <form method="POST" action="{{ route('pages.store') }}">
                ...
                <div class="flex gap-2">
                    <input type="text" name="judul" placeholder="Nama Halaman (ex: Kas Masuk)" class="flex-1 border p-2 rounded" required>
            ...
            <table class="w-full text-left">
                ...
                <tbody>
                    @forelse($pages as $page)
                        <tr class="border-b">
                            <td class="p-3 font-bold">{{ $page->judul }}</td>
                            <td class="p-3">
                                @if($page->tipe == 'kas') <span class="bg-green-100 text-green-800 px-2 rounded">Kas</span>
                                @elseif($page->tipe == 'barang_pinjam') <span class="bg-yellow-100 text-yellow-800 px-2 rounded">Barang</span>
                                @else <span class="bg-gray-100 text-gray-800 px-2 rounded">Info</span>
                                @endif
                            </td>
                            <td class="p-3">
                                <form method="POST" action="{{ route('pages.destroy', $page->id) }}" onsubmit="return confirm('Hapus?')">
                                    @csrf
                                    @method('DELETE')
