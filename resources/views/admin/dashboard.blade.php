...
<div class="min-h-screen py-12 px-4 sm:px-6 lg:px-8 relative z-10">
    <div class="max-w-6xl mx-auto">
        ...
        <!-- Grid Room Cards (Styled like my-rooms) -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            @forelse($tempatLayanans as $item)
                <div class="glass-card rounded-[2rem] p-8 flex flex-col justify-between shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-white/10 relative overflow-hidden group">
                    ...
                    <!-- Action Buttons -->
                    <div class="flex items-center gap-3 mt-auto pt-4 border-t border-white/5 relative z-10">
                        <!-- Tombol Edit -->
                        <a href="{{ route('admin.temp.edit', $item->id) }}" class="p-3 bg-blue-500/10 text-blue-400 rounded-xl hover:bg-blue-500 hover:text-white transition-all border border-blue-500/20 hover:border-blue-500" title="Edit Pengaturan">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                        </a>
                        
                        <!-- Tombol Anggota -->
                        <a href="{{ route('admin.group.members', $item->id) }}" class="p-3 bg-emerald-500/10 text-emerald-400 rounded-xl hover:bg-emerald-500 hover:text-white transition-all border border-emerald-500/20 hover:border-emerald-500" title="Lihat Anggota Grup">
                        ...
                        <!-- Tombol Pages -->
                        <a href="{{ route('admin.pages.index', $item->id) }}" class="p-3 bg-purple-500/10 text-purple-400 rounded-xl hover:bg-purple-500 hover:text-white transition-all border border-purple-500/20 hover:border-purple-500" title="Kelola Halaman">
                        ...
                        <form action="{{ route('admin.temp.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus permanen?');" class="ml-auto">
                            @csrf @method('DELETE')
                            <button type="submit" class="p-3 bg-red-500/10 text-red-400 rounded-xl hover:bg-red-500 hover:text-white transition-all border border-red-500/20 hover:border-red-500" title="Hapus Layanan">
        ...
        <p class="mt-12 text-center text-gray-500 text-xs font-medium uppercase tracking-widest">
